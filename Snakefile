...
rule all:
	...
	# DADI
		build_sfs_subpops = expand("models/dadi/sfs/{model}.fs", model = ['cap_gem_vir', 'sab_alb_palm', 'ital_botr']),
		build_sfs_wild_dom = expand("models/dadi/sfs/{model}.fs", model = ['wild_domesticated']),
		build_sfs_kales = expand("models/dadi/sfs/{model}.fs", model = ['gong_ital_kale']), 
		run_dadi_inference = expand("models/dadi/results/{model}.csv", model = ['cap_gem_vir', 'sab_alb_palm', 'ital_botr', 'wild_domesticated', 'gong_ital_kale']),
		bootstrap_sfs_subpops = expand("models/dadi/sfs_bootstrap/{model}_{rep}.fs", model = ['cap_gem_vir', 'sab_alb_palm', 'ital_botr'], rep = range(0,9)),
		bootstrap_sfs_wild_domesticated = expand("models/dadi/sfs_bootstrap/{model}_{rep}.fs", model = ['wild_domesticated'], rep = range(0,9)),
		bootstrap_sfs_wild_kale = expand("models/dadi/sfs_bootstrap/{model}_{rep}.fs", model = ['gong_ital_kale'], rep = range(0,9))
